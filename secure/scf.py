...
class SB110Model(mesa.Model):
    ...
    def __init__(
        ...
        self.filename = f"C:\\Users\\sante\\MyProject\\scf\\secure\\venv\\dataset\\scf_simulation_{timestamp}.csv"
        # Create the week number counter
        self.week = 0
        # Write the header to the file
        with open(self.filename, 'a', newline='') as file:
            writer = csv.writer(file)
            header = ["Week", "Financial", "Tech1", "Tech2", "GC1", "GC2", "Client1", "Client1", "Paperwork1",
                      "Paperwork2", "Paperwork3", "Paperwork4", "Paperwork5"]
    ...
    def init_test(
        ...
        gc_general_2 = GeneralContractorAgent(
            5,
            self,
            "DISIM Costruzioni Spa",
            dm,
            "xxx",
            "yyy",
            gc_threshold,
    ...
    def step(self):
        ...
        """
        # Check if all Paperworks are archived, if all Archived exit the program simulation
        arr = []
        for i in range(8,12):
            print(i)
            print(DataManagerAgent.get_pwork_status_from_dict(str(i)))
            arr.append(DataManagerAgent.get_pwork_status_from_dict(str(i)))  #get_pwork_status_dict DataManagerAgent.get_pwork_status_from_dict()
            print(arr)
        print(arr)
        if len(set(arr)) == 1 and arr[0] == "Archived":
            print("All paperwork are Archived. Exiting the program.")
            sys.exit()
        """
    ...
    def compute_gc(model):
        ...
        # add data to file
        with open(model.filename, 'a', newline='') as file:
            data = agent_wealths # Fetch updated wallets
            data.insert(0,model.week) # Insert the week number
            writer = csv.writer(file)
            writer.writerow(data) # Write updated wallet values
            model.week += 1 # Increase the week counter
        return avg
...
class GeneralContractorAgent(mesa.Agent):
    ...
    def step(self):
        ...
        for agent in self.dm.get_pwork_list():
            # Check if Paperwork is assigned to current General Contractor and is not Archived
            status = str(self.dm.get_gc_payment_from_dict(agent.unique_id))
            if agent.general_contractor == self.unique_id and status != "Archived":
                ...
                if self.choose:  # Success of approval, GC Agent pays the work done
                    ...
                    if new_status != "Archived":
                        ...
                        print(
                            "Amount sent: "
                            + str(amount)
                            + " Paperwork Account blance: " + str(agent.wealth) # microAlgos".format(pwork_account_info.get("amount")
                            + "\n"
                        )
                        print(
                            f"Paperwork New payment state is: {new_status}" + " new paperwork balance is: " + str(agent.wealth))
                        """
                        ...
                        print(
                            f"Paperwork New payment state is: {new_status}" + " Project is finished")
                    print(
                        "-------------------------------------------------------------------------------------------"
                    )
                else:
        ...
        if status == "Archived":
            ...
            for agent in self.dm.get_tech_list():
                ...
                if agent.unique_id == self.technician:
                    agent.wealth += tech_amount # aggiornato sul wealth dell'agente tech viene aggiunto l'importo dell'asseverazione
                    """
